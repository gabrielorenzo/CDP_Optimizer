...
from ISP.modules.basic_module import MODULE_DEPENDENCIES
...
class Pipeline:
    ...
    def execute(self, bayer, save_intermediates=True, verbose=True):
        """
        ...
        data['output'] = self.get_output(data) #returns ycbcr image with format 1948,2592,3. This is not the format that we want
    ...
    def run(self, raw_path, save_dir, load_raw_fn, suffix=''):
        ...
        data, _ = self.execute(bayer, save_intermediates=True, verbose=False) #save intermediates to disk, set to true so that we can see intermediate results
